            ...
            if (result) {
                try {
                    ...
                    if (jsonObject.getString(Constant.STATUS).equals(Constant.SUCCESS)) {
                        JSONObject notices = jsonObject.getJSONObject(Constant.NOTICES);

                        String first_page_url = notices.getString("first_page_url");
                        String last_page_url = notices.getString("last_page_url");
                        String next_page_url = notices.getString("next_page_url");
                        String prev_page_url = notices.getString("prev_page_url");
                        ...
                        JSONArray data_array = notices.getJSONArray(Constant.DATA);
                        for (int i = 0; i < data_array.length(); i++) {
                            ...
                            BlogModel blogModel = new BlogModel(id, title, "notice", images, description, status, slug, created_at, updated_at);
                    ...
                    e.printStackTrace();
                    Log.d("eNotices", e.getMessage());
                }
            }
            Log.d("rNotices", response);
